    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div class="d-flex align-items-center flex-wrap gap-2">
            ...
            if (!empty($selectedClients)) {
                $client_names = [];
                $ids = implode(",", array_map('intval', $selectedClients));
                $res = mysqli_query($conn, "SELECT first_name FROM client WHERE id IN ($ids)");
                while ($row = mysqli_fetch_assoc($res)) {
                    $client_names[] = htmlspecialchars($row['first_name']);
                }
                if (!empty($client_names)) {
                    $active_filters[] = "Client: " . (count($client_names) > 2 ? 
                        implode(", ", array_slice($client_names, 0, 2)) . " +" . (count($client_names) - 2) : 
            ...
            if (!empty($selectedCountries)) {
                $country_names = [];
                $ids = implode(",", array_map('intval', $selectedCountries));
                $res = mysqli_query($conn, "SELECT name FROM countries WHERE id IN ($ids)");
                while ($row = mysqli_fetch_assoc($res)) {
                    $country_names[] = htmlspecialchars($row['name']);
                }
                if (!empty($country_names)) {
                    $active_filters[] = "Country: " . (count($country_names) > 2 ? 
                        implode(", ", array_slice($country_names, 0, 2)) . " +" . (count($country_names) - 2) : 
                        implode(", ", $country_names));
                }
            }
            ...
            if (!empty($selectedBalances)) {
                ...
                foreach ($selectedBalances as $balance) {
                    $balance_labels[] = '$' . number_format($balance, 2);
                }
                if (!empty($balance_labels)) {
                    $active_filters[] = "Balance: " . (count($balance_labels) > 2 ? 
                        implode(", ", array_slice($balance_labels, 0, 2)) . " +" . (count($balance_labels) - 2) : 
                        implode(", ", $balance_labels));
                }
            }
            
            // Date range filter
            if (!empty($dateRange)) {
                $active_filters[] = "Date: " . htmlspecialchars($dateRange);
            }
            ?>
