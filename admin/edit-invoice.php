    ...
    <table class="table mb-0 add-table <?= $gst_type == 'non_gst' ? 'non-gst-mode' : '' ?>">
        ...
        <tbody class="add-tbody" id="product">
            ...
            $item_query = "SELECT 
                                ...
                                IFNULL(qi.service_name, '') AS service_name
            ...
            while ($item = mysqli_fetch_assoc($item_result)) {
                ...
                $row_class = $item_type == 1 ? 'product-row' : 'service-row';
                $qty = (float)($item['quantity'] ?? 0);
                $price = (float)($item['item_selling_price'] ?? 0);
                $taxRate = (float)($item['tax_rate'] ?? 0);
                $taxName = htmlspecialchars($item['tax_name'] ?? '');
                $taxId = (int)($item['tax_id'] ?? 0);
                $hsnCode = htmlspecialchars($item['hsn_code'] ?? '');
                $service_name = htmlspecialchars($item['service_name'] ?? '');
                
                // Adjust tax for non-GST mode
                $effectiveTax = $gst_type == 'non_gst' ? 0 : $taxRate;

                $lineSubtotal = $qty * $price;
                $lineTaxAmount = $lineSubtotal * $effectiveTax / 100;
                $amount = $lineSubtotal + $lineTaxAmount;
                ?>
                <tr class="<?= $row_class ?>">
                    <td>
                        <div class="product-fields">
                            ...
                            <input type="hidden" class="tax-name" name="tax_name[]" value="<?= $taxName ?>">
                        </div>
                        <div class="service-fields">
                            <select class="form-select service-select" name="item_id[]">
                                ...
                                <option value="<?= $service['id'] ?>" 
                                    ...
                                    <?= ($service['id'] == $item['product_id']) ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($service['name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="text" class="form-control service-name-input service-custom-input" name="service_name[]" placeholder="Or enter custom service name" value="<?= $service_name ?>">
                            <input type="hidden" class="tax-id" name="tax_id[]" value="<?= $taxId ?>">
                            <input type="hidden" class="tax-name" name="tax_name[]" value="<?= $taxName ?>">
                        </div>
                    </td>
                    <td>
                        <input type="number" class="form-control quantity <?= $item_type == 0 ? 'service-quantity' : '' ?>" name="quantity[]" value="<?= $qty ?>" <?= $item_type == 1 ? 'min="1"' : 'placeholder="Optional"' ?>>
                    </td>
                    <td>
                        <input type="text" class="form-control hsn-code" name="code[]" value="<?= $hsnCode ?>" readonly>
                    </td>
                    <td>
                        <div class="product-fields">
                            <input type="text" class="form-control selling-price" name="selling_price[]" value="<?= '$ ' . number_format($price, 2) ?>" data-value="<?= $price ?>" readonly>
                        </div>
                        <div class="service-fields">
                            <input type="text" class="form-control service-price-input" name="selling_price[]" value="<?= '$ ' . number_format($price, 2) ?>" data-value="<?= $price ?>" placeholder="0.00">
                        </div>
                    </td>
                    <td class="tax-column">
                        <div class="product-fields">
                            <input type="text" class="form-control tax-rate" name="rate[]" value="<?= number_format($effectiveTax, 2) . '%' ?>" data-value="<?= $effectiveTax ?>" style="display: none;">
                            <div class="tax-display-container">
                                <div class="tax-amount-line">$ <?= number_format($lineTaxAmount, 2) ?></div>
                                <div class="tax-rate-line"><?= number_format($effectiveTax, 2) ?>%</div>
                            </div>
                        </div>
                        <div class="service-fields">
                            <select class="form-select service-tax-select" name="tax_id[]">
                                ...
                                <option value="<?= $tax['id'] ?>" data-rate="<?= $tax['rate'] ?>" <?= ($tax['id'] == $taxId) ? 'selected' : '' ?>>
                                    <?= $tax['name'] ?> (<?= $tax['rate'] ?>%)
                                </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="hidden" class="tax-rate" name="rate[]" data-value="<?= $effectiveTax ?>" value="<?= $effectiveTax ?>">
                            <input type="hidden" class="tax-name" name="tax_name[]" value="<?= $taxName ?>">
                    ...
                    <td>
                        <input type="text" class="form-control amount" name="amount[]" value="<?= '$ ' . number_format($amount, 2) ?>" data-value="<?= $amount ?>" readonly>
                    </td>
                    <td>
                                            ...
                                            <div class="row">
                                                ...
                                                <input type="hidden" name="total_amount" id="total-amount-field" value="<?= $row['total_amount'] ?>">
                                                    ...
                                                    <div class="mb-3">
                                                       ...
                                                       <div class="tax-details mb-3 <?= $gst_type == 'non_gst' ? 'd-none' : '' ?>">
...
/* =========================
   Client info fetch - FIXED VERSION
========================== */
